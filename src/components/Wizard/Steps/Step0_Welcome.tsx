...
export function Step0({ onNext }: { onNext: () => void }) {
  ...
  return (
    <div className="relative w-full">
      {/* Top Navigation Bar */}
      <div
        className="relative w-full flex flex-col sm:flex-row justify-center items-center z-30 pt-10 sm:pt-8 pb-4 gap-4 sm:gap-6 px-12 sm:px-4"
        style={{ direction: "rtl" }}
      >
        ...
        >
          ...
          <div className={cn("space-y-4 overflow-visible")}>
            ...
            <p
              ...
              style={{
                fontFamily: partner?.slogan_font_family
                  ? FONT_FAMILY_OPTIONS[partner.slogan_font_family].css
                  : "inherit",
              }}
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              ...
              <div
                key={i}
                className="relative flex items-center gap-3 sm:gap-4 p-3 sm:p-4 rounded-xl bg-white border border-slate-100 shadow-sm hover:shadow-md transition-all group h-auto min-h-[80px] md:h-[90px]"
              >
                ...
                <div
                  className={cn(
                    "flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center group-hover:scale-110 transition-all shadow-inner",
                    step.bg,
                  )}
                >
      ...
      {/* Partner Login — bottom of page, visible only to non-authenticated users */}
      {!isAdmin && !isOwner && (
        <div className="flex justify-center pb-6 pt-2">
          <Button
            ...
            asChild
          >
            <Link to="/login">{t.partnerLogin}</Link>
          </Button>
        </div>
      )}
    </div>
